...
const HoroscopeContent = () => {
  ...
  const { name, dob, tob, pob, lat, lon } = useMemo(() => {
    return {
      name: sanitize(searchParams.get('name'), 100) || 'Guest',
      dob: sanitize(searchParams.get('dob'), 10),
      tob: sanitize(searchParams.get('tob'), 5),
      pob: sanitize(searchParams.get('pob'), 100),
      lat: sanitizeCoord(searchParams.get('lat')),
      lon: sanitizeCoord(searchParams.get('lon')),
    };
  }, [searchParams]);
