...
function calculatePlanetaryAndDivisionalData(
    ...
    let lagnaSidereal: number;
    if (kpLagnaOverride !== undefined) {
        lagnaSidereal = kpLagnaOverride;
    } else {
        ...
        lagnaSidereal = (lagnaTropical - ayanamsa + 360) % 360;
    }
...
export function calculateAllShadBala(
    ...
    for (const pName of targetPlanets) {
        ...
        // 2. Saptavargaja Bala
        const saptavargajaBala = calculateSaptavargajaBala(pName, long, planetMap);
...
export function generateAstrologyData(dob: string, tob: string, latStr?: string, lonStr?: string, kpHoraryNumber?: number): ChartData {
    ...
    const getCoreData = () => {
        if (!coreData) {
            ...
            coreData = calculatePlanetaryAndDivisionalData(time, lat, lon, ayanamsa, getTropicalSunLong(), getTropicalMoonLong(), getRotEqjEct(), kpLagnaOverride);
        }
        return coreData;
    };
...
export function getCompoundRelationship(planet: string, lord: string, planetMap: Record<string, PlanetData>): string {
    if (planet === lord) return "Own";
    const pData = planetMap[planet];
    const lData = planetMap[lord];
...
export function calculateSaptavargajaBala(pName: string, long: number, planetMap: Record<string, PlanetData>): number {
    ...
    for (const v of vargas) {
        ...
        if (isExalted) {
            ...
            const rel = getCompoundRelationship(pName, signLord, planetMap);
...
export {
    getPlanetTransits,
    getTransitsPerAscendant,
    getRetrogradeDetails,
    getCombustionDetails,
    getFutureCombustions,
    isPlanetCombustAt,
    getSiderealLagnaAt,
    type TransitEvent,
    type PlanetTransits,
    type CombustionPeriod,
    type PeriodDetails,
    type TransitPeriodGroup,
    type AscendantTransitData
} from './transits';
