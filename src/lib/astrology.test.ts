...
test('Traditional Dasha Year configuration correctness', () => {
    ...
    // 1. Test 360-day Savana Year
    setDashaYearDays(360);
    const dasha360 = calculateVimshottariDasha(moonLongitude, birthDate);

    // 2. Test 365.25-day Calendar Year
    setDashaYearDays(365.25);
    const dasha365 = calculateVimshottariDasha(moonLongitude, birthDate);

    assert.ok(dasha360[0].end !== dasha365[0].end, "Dates should differ between 360 and 365.25 day configurations");

    // Cleanup to default
    setDashaYearDays(360);
});

test('Ayanamsa configuration and precise offsets correctness', () => {
    const dob = "1990-01-01";
    const tob = "12:00";
    const lat = "28.6139";
    const lon = "77.2090";
    ...
    // Standard Lahiri
    setAyanamsa(Ayanamsa.Lahiri);
    const dataLahiri = generateAstrologyData(dob, tob, lat, lon);

    // Raman
    setAyanamsa(Ayanamsa.Raman);
    const dataRaman = generateAstrologyData(dob, tob, lat, lon);

    // Krishnamurti (KP)
    setAyanamsa(Ayanamsa.Krishnamurti);
    const dataKP = generateAstrologyData(dob, tob, lat, lon);

    // Fagan-Bradley
    setAyanamsa(Ayanamsa.FaganBradley);
    const dataFagan = generateAstrologyData(dob, tob, lat, lon);

    // Confirm they compute distinct values
    const moonLahiri = dataLahiri.planets.find(p => p.name === "Moon")?.degree;
    const moonRaman = dataRaman.planets.find(p => p.name === "Moon")?.degree;
    const moonKP = dataKP.planets.find(p => p.name === "Moon")?.degree;
    const moonFagan = dataFagan.planets.find(p => p.name === "Moon")?.degree;

    assert.ok(moonLahiri !== moonRaman, "Lahiri and Raman should produce different sidereal positions");
    assert.ok(moonLahiri !== moonKP, "Lahiri and KP should produce different sidereal positions");
    assert.ok(moonLahiri !== moonFagan, "Lahiri and Fagan-Bradley should produce different sidereal positions");

    // Cleanup
    setAyanamsa(Ayanamsa.Lahiri);
});

test('Zero cumulative drift in nested dasha calculations', () => {
    const moonLongitude = 306.475;
    const birthDate = new Date('1990-01-01T12:00:00Z');
    setDashaYearDays(360);

    const dasha = calculateVimshottariDasha(moonLongitude, birthDate);
    const firstMahadasha = dasha[0];

    // Compute sum of Antardasha durations to verify it perfectly matches the parent Mahadasha end
    const antardashas = firstMahadasha.antardashas;
