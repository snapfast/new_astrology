...
export function sanitize(val: unknown, maxLength: number): string {
  ...
  let sanitized = val
    ...
    // Includes \u2028 and \u2029 as they can break JS strings and be used for obfuscation.
    .replace(
      /[\x00-\x1F\x7F\u00AD\u180E\u200B-\u200F\u2028\u2029\u202A-\u202E\u2060\u2066-\u2069\uFEFF]/g,
      "",
    )
  ...
  do {
    ...
    sanitized = sanitized.replace(
      /(javascript|vbscript|data|feed|file|jar|srcdoc|about|content|ms-appx|ms-appx-web|blob|filesystem)\s*:/gi,
      "",
    );
  ...
  return (
    sanitized
      ...
      .replace(
        /\b(on\w+|style|formaction|action|background|poster|srcdoc)\b[\s/]*=/gi,
        "",
      )
      .trim()
  );
