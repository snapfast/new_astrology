...
const HouseContent: FC<HouseContentProps> = ({ x, y, rasi, planets }) => {
  return (
    <g>
      ...
      >
        <div className={`w-fit mx-auto h-full font-medium text-[14px] ${planets?.length >= 3 ? 'grid grid-cols-2 gap-x-0.5 gap-y-0.5 place-items-center place-content-center leading-none' : 'flex flex-row justify-center items-start gap-0.5'}`}>
          {(planets || []).map((p, i) => {
